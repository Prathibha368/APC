...
int main(int argc, char *argv[])
{
	...
	if(cla_validation(argc,argv)==FAILURE){
        ...
        printf(YELLOW "Usage: ./a.out <op1> <op> <op2>\n" RESET);
		return 0;
	}
	...
	printf(BLUE "\nOperand 1 : " RESET "%s\n", argv[1]);
    printf(BLUE "Operand 2 : " RESET "%s\n", argv[3]);
    printf(BLUE "Operator  : " RESET "%c\n", oper);
    ...
    switch(oper)
    {
		case '+':  //for addition
		{
			printf(YELLOW "Operation chosen : ADDITION (+)\n" RESET);
		...
		{
			printf(YELLOW "Operation chosen : SUBTRACTION (-)\n" RESET);
		...
		case 'X':
		{
			printf(YELLOW "Operation chosen : MULTIPLICATION (x)\n" RESET);
			if(negative_flag1&&negative_flag2)
				neg=0;
		...
		case '/': //for division
		{
			printf(YELLOW "Operation chosen : DIVISION (/)\n" RESET);
			if(head2->data==0){
